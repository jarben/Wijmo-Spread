...
function explorerVM() {
	var self = this;
	self.theme = ko.observable("../css/cobalt/jquery-wijmo.css");
	...
	self.widgetList = function () {
		...
		for (var propertyName in self.widgetListObject) {
			...
			self.widgetListObject[propertyName].imageUrl = 'explore/images/widget/' + propertyName + '.png';
    ...
    ////filter widgetList array based isUseCase boolean
    self.widgetUseCases = function () {
        var cases = [];
        $.each(self.widgetList(), function(i, item){
            if(item.isUseCase == true) {
                cases.push(item);
            }
        });
...
//		}
        ...
        if(href.indexOf("index.html")) {
            pathArray = href.substring(0, href.indexOf("index.html"));
        } else if(href.indexOf("?widget=")) {
            pathArray = href.substring(0, href.indexOf("?widget="));
        } else {
           pathArray = href;
        }
		...
		$.ajax({
			...
			success: function (data) {
				...
				var callback = function () {
                    ...
                    //Fixed when change to sample, spread refresh issue.
                    $("#ctl00_WidgetTabs").wijtabs("select", 0);
                    var spread = $("#ss").wijspread("spread");
                    if(spread && spread.repaint) {
